	...
	<div class="contain">
        ...
        <el-table-column fixed="right" label="审核" width="110">
					<template slot-scope="scope">
						<el-select v-model="status" placeholder="未审核" @change="audit(scope.row,status)" v-if="scope.row.projectCheck==1">
					    	<el-option
					      		v-for="item in options" :key="item.value" :label="item.label" :value="item.value" >
					    	</el-option>
				  		</el-select>
              <h5 v-if="scope.row.projectCheck!=1">{{scope.row.projectCheckTxt}}</h5>
					</template>
				</el-table-column>
