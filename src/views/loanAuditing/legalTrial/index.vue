        ...
        <el-table-column prop="applyTime" label="申请时间" ></el-table-column>
          <el-table-column fixed="right" label="审核" width="110">
					<template slot-scope="scope">
						<el-select v-model="status" @change="audit(scope.row,status)">
					    	<el-option
      ...
      API.postLoanAudit(params).then(result => {
        ...
        for(let i=0;i<list.length;i++){
          if(list[i].applyStatus=='1'){
            list[i].applyStatusTxt='未审核';}
            if(list[i].applyStatus=='2'){
            list[i].applyStatusTxt='通过';}
            if(list[i].applyStatus=='3'){
            list[i].applyStatusTxt='拒绝';}
        }
        self.tableData = list;
      });
    },
